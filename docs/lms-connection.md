...
About the default configuration, if you check `RICHIE_LMS_BACKEND` settings in `env.d/development`
you will see that we use `base.BaseLMSBackend` as `RICHIE_LMS_BACKEND`.
...
If you want to develop with OpenEdx as `RICHIE_LMS_BACKEND` of Richie, you need to enable TLS for your
